...
set STRUCTURES=TYPMAS UNTMAS

set USE_STRUCTURE_ALIASES=-a UNIT_TYPE UNIT

set ENABLE_CLEAN_DATA=-define CLEAN_DATA
set USE_ALTERNATE_FIELD_NAMES=-af
...
set STDOPTS=-e -r -lf -u UserDefinedTokens.tkn %USE_STRUCTURE_ALIASES% %USE_ALTERNATE_FIELD_NAMES% %ENABLE_CLEAN_DATA%

rem ---------------------------------------------------------------------------
rem Generate code

rem Generate SQL I/O routines for the structures being replicated
codegen  -s %STRUCTURES% -t SqlIO CosmosIO %STDOPTS%
